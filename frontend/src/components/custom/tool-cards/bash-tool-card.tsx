...
export function BashToolCard({ toolCall }: ToolCardContentProps) {
  return (
    <div className="space-y-2">
      {toolCall.args.command && (
        <code className="block text-xs font-mono bg-muted/50 px-2 py-1.5 rounded whitespace-pre-wrap break-all">
          {toolCall.args.command}
        </code>
      )}
      {toolCall.error && (
