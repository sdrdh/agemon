...
self.addEventListener('push', (event) => {
  ...
  const options: NotificationOptions = {
    ...
    data: {
      ...
      resolveToken: data.resolveToken,  // Short-lived, single-use token for auth
    },
  };
...
- **Service worker auth** — the notification resolve endpoint does NOT use the long-lived `AGEMON_KEY`. Instead, each push payload includes a short-lived, single-use `resolveToken` generated by the server. The server stores the token hash in the `notifications` table. Tokens expire after 10 minutes or first use. This avoids storing secrets in the service worker's global scope.
