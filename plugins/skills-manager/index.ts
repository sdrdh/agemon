...
export const plugin: PluginModule = {
  onLoad(ctx: PluginContext): PluginExports {
    ...
    // IMPORTANT: /skills/preview MUST be before /skills/:name
    apiRoutes.post('/skills/preview', async (c) => {
    ...
    apiRoutes.delete('/skills/:name', async (c) => {
    ...
    apiRoutes.get('/tasks/:taskId/skills', async (c) => {
    ...
    apiRoutes.post('/tasks/:taskId/skills', async (c) => {
      const taskId = c.req.param('taskId');
      assertSafeParam(taskId, 'task ID');
      const taskDir = join(agemonDir, 'tasks', taskId);
      const body = await c.req.json();
    ...
    apiRoutes.delete('/tasks/:taskId/skills/:name', async (c) => {
      const taskId = c.req.param('taskId');
      const name = c.req.param('name');
      assertSafeParam(taskId, 'task ID');
      assertSafeParam(name, 'skill name');
