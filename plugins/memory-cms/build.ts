/**
 * Build script for memory-cms plugin renderers.
 *
...
if (files.length === 0) {
  console.info('[memory-cms] no renderers to build');
  process.exit(0);
}
...
if (!result.success) {
  console.error('[memory-cms] build failed:');
...
console.info(`[memory-cms] built ${result.outputs.length} renderer(s)`);
