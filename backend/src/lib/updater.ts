...
// ─── Shared: install deps + rebuild frontend ────────────────────────────────
...
async function installAndBuild(
  ...
  if (!build.ok) {
    // Non-fatal — backend still works, just serves stale UI
    console.warn(`[updater] frontend build failed: ${build.stderr}`);
    return { ...base, ok: true, message: `${successMessage} (warning: frontend build failed, UI may be stale)` };
  }
...
async function applyTagUpdate(
  ...
  return installAndBuild(base, rollbackRef, `Updated from ${CURRENT_VERSION} to ${targetTag}`);
}
...
async function applyBranchUpdate(
  ...
  return installAndBuild(
    { ...base, to_version: `${branch}@${newHead}` },
    rollbackRef,
    `Switched to branch ${branch} (${newHead})`,
  );
}
